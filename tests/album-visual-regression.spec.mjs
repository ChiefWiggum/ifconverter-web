        ...
        cases: [
            {
                label: 'pages-cover',
                indices: [0],
                referenceFile: 'reference-pages-cover.png',
                maskCenterStrip: false,
                maskRects: COVER_VIEWER_MASKS
            },
    ...
    },
    {
        ...
        cases: [
            ...
            { label: 'pages-58-59', indices: [30], referenceFile: 'reference-pages-58-59.png' },
            {
                label: 'pages-60',
                indices: [31],
                referenceFile: 'reference-pages-60.png',
                maskCenterStrip: false
            }
        ]
    }
...
async function compareToReference(
    ...
    let actual = await sharp(screenshotBuf)
        .resize(compareW, compareH)
        .ensureAlpha()
        .raw()
        .toBuffer();
    let expected = await sharp(referencePath)
        .resize(compareW, compareH)
        .ensureAlpha()
        .raw()
        .toBuffer();
    ...
    const diffPixels = pixelmatch(actual, expected, null, compareW, compareH, {
    ...
    test.describe(`${album.title} visual comparisons`, () => {
        ...
        for (const testCase of album.cases) {
            test(`${testCase.label} match reference >=95%`, async ({ page }) => {
                ...
                const screenshotBuf = await loadAlbumAndRenderSelection(
                    page,
                    album.albumPath,
                    testCase.indices
                );
